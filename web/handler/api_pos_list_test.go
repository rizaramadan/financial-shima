...
func apiPosTestServer(t *testing.T) *echo.Echo {
	t.Helper()
...
func TestAPIPosList_Returns401_NoAPIKey(t *testing.T) {
	t.Parallel()
	e := apiPosTestServer(t)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/pos", nil)
...
func TestAPIPosList_NilDB_Returns503(t *testing.T) {
	t.Parallel()
	e := apiPosTestServer(t)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/pos", nil)
