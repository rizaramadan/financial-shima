...
| 9 | Web UI: views (§6.1–6.5) | **stages A+B+C+D+E done** | A: §6.2 Home. B: §6.5 Notifications. C: bell badge. D: §6.1 Transactions list. E: §6.3 Pos detail — name/currency/target header, balance card (cash em-dash until ledger creation handler wires; receivables/payables aggregated from open pos_obligation rows), open-obligations table with direction/counterparty/outstanding/since, scoped transaction list with reversal styling. NotFound + LoadError render distinct states. Routes: GET /pos/:id. 3 unit tests pin: unauth redirect, invalid-uuid redirect to /, nil-pool not-found render. F (spending §6.4) is the last view. |
