...
| 9 | Web UI: views (§6.1–6.5) | **stages A+B+C done** | A: §6.2 Home. B: §6.5 Notifications feed. C: bell badge in layout header — server-rendered (full-page-poll, no JS / no third-party CDN; trade-off: count refreshes on navigation, not on a 30s timer). HomeGet + NotificationsGet populate `UnreadCount` once per render via a shared `loadBellCount` helper. Layout `{{if .SignedIn}}` guard suppresses the bell on /login and /verify. Tests pin: bell hidden on login page, bell present + linked to /notifications on home, badge empty when count is zero. D (transactions list §6.1), E (pos detail §6.3), F (spending §6.4) follow. |
