...
| 9 | Web UI: views (§6.1–6.5) | **stages A+B+C+D done** | A: §6.2 Home. B: §6.5 Notifications feed. C: server-rendered bell badge. D: §6.1 Transactions list — date-range filter (`?from=YYYY-MM-DD&to=…`, default last 30 days), invalid dates silently fall back, ListTransactionsByDateRange joins account+pos+counterparty names + posts the data into a flattened TransactionRow slice. Reversal rows render with strikethrough + a "reverses" badge linking back to the original. Multi-select filters (account/pos/cp/type) deferred to a follow-up. 4 unit tests pin: unauth redirect, nil-pool empty-state, query-param round-trip, invalid-date fallback. E (pos detail §6.3), F (spending §6.4) follow. |
