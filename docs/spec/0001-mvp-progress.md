...
| 9 | Web UI: views (§6.1–6.5) | **stages A+B done** | A: §6.2 Home (accounts + pos grouped by currency, IDR-first, em-dash for unwired balances, LoadError alert vs empty-state placeholder, Skeet 9 / Beck 9). B: §6.5 Notifications feed (newest-first, unread visually distinct, "Mark read" per-row + "Mark all read" bulk, relative-time formatter, link to /transactions/:id when set). DB-tolerant nil-pool path renders empty state. 4 unit tests pin redirect-on-unauth + nil-pool empty-state. C (HTMX bell badge poll), D (transactions list §6.1), E (pos detail §6.3), F (spending §6.4) follow. |
