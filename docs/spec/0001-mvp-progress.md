...
| 9 | Web UI: views (§6.1–6.5) | **stage A: home view** | §6.2 Home renders accounts (table) + pos grouped by currency from the wired pgx pool. handler.New gained an optional *pgxpool.Pool; nil pool falls back to the placeholder render so cmd/server tests stay DB-free. db/seed/dev.sql idempotent fixture for local dev. Integration test covers Accounts label + a seeded Pos name + the gold-g currency group + the logout form. Balance computation from transactions wires once the ledger has a creation handler. Other views (transactions, pos detail, spending, notifications) ship in subsequent sub-stages. |
